...
const STAGES = new Set(["all", "poller", "fmv"]);
const stage = process.argv[2] || "all";
if (!STAGES.has(stage)) {
  console.error("unknown stage '" + stage + "' — expected one of: all, poller, fmv");
  process.exit(1);
}
...
// Unwrap a Runtime.evaluate response, surfacing in-frame exceptions instead
// of returning undefined and masking the real error.
const unwrap = (res) => {
  if (res.exceptionDetails) {
    const ex = res.exceptionDetails.exception?.description || res.exceptionDetails.text;
    throw new Error("in-frame evaluation failed: " + ex);
  }
...
try {
  ...
  const probe = async () =>
    unwrap(
      await evalIn(cdp, sid, "({polled: (window.__FMV_rt || []).length, hasFMV: !!window.FMV, menu: !!(window.FMV && window.FMV.menu)})")
    );
  ...
  if (stage === "all" || stage === "fmv") {
    if (!p.hasFMV) {
      ...
      const hunt = unwrap(await evalIn(cdp, sid, HUNTER_SOURCE));
      console.log("hunter:", JSON.stringify(hunt));
      if (!hunt || hunt.ok !== true) throw new Error("hunter failed — is the farm fully loaded?");
      const res = await evalIn(cdp, sid, FMV_HELPER_SOURCE);
      console.log("FMV install:", JSON.stringify(unwrap(res)));
    }
  }

  if (stage === "all") {
    const menu = await evalIn(cdp, sid, MENU_SOURCE);
    console.log("menu install:", JSON.stringify(unwrap(menu)));
