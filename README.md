...
Version: 1.6.0 · Date: 2026-08-08 · Discord Activity build (served from `1187013846746005515.discordsays.com`, CrazyGames proxy)
...
- Fix: everything is discovered structurally at runtime:
  - `src/poller.js` captures the require of **every** webpack runtime that processes a fake
    chunk into `window.__FMV_rt` — no hardcoded runtime name.
  - `src/hunter.js` picks the runtime whose module store has the most executed modules, then
  ...
  - `src/fmv_helper.js` (v4) builds `window.FMV` from the discovered layout.
...
Discord activity iframe gets its own CDP target). `src/cdp_lib.mjs` reads the MCP profile's
...
| File | Purpose |
|---|---|
| `src/cdp_lib.mjs` | CDP client over Node's built-in WebSocket; finds Chrome's debug port (`DevToolsActivePort` candidates); attach + evaluate helpers; `findGameTarget()` (discordsays iframe) |
| `src/poller.js` | Generic poller injected into the game frame; captures all webpack runtime requires into `window.__FMV_rt` |
| `src/hunter.js` | In-frame module hunter: picks the main runtime, re-discovers root container / farm services / component map / MergeTrigger ctor for the current build |
| `src/fmv_helper.js` | `window.FMV` v4 (board / merge / move / swap / spawnCrate / services / req / I / root / rootServices) |
| `src/menu.js` | In-game bot menu (FMV Bot overlay, top-right of the game window): draggable panel with Sort / Fill / Harvest / Plan+Merge / Auto Orders / Orders / Refresh + status + log. All bot logic runs in-frame; exposes `window.FMV.menu` |
| `src/install.mjs` | One-shot installer — poller (if missing) → hunter (if missing) → FMV helper → menu overlay |
...
- Debug stages: `node src\install.mjs poller` (poller only) / `node src\install.mjs fmv` (hunter + FMV only).
...
- **Re-run `src/install.mjs` after any Chrome restart / Discord activity restart / game
  reload** — the injection lives in the frame and does not survive a reload.

### CLI-only alternatives (debugging / scripting)

- `node src\eval.mjs "…"` — one-off evals; `node src\eval.mjs "window.FMV.merge(c,r,c,r)"` for a single merge.
...
- **Version strings must stay in sync**: on every release bump, update the
  `Project version` in `AGENTS.md`, `CHANGELOG.md`, this `Version:` line, and
  the `VERSION` constant in `src/install.mjs` — the installer injects
  `window.__FMV_version` and `fmv_helper.js` / `menu.js` read it, so the
  in-game header follows automatically.
...
- Run `src/install.mjs` only after the farm is fully loaded; the menu shows a clear error if
